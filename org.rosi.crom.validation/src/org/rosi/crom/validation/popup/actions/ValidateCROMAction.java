...
public class ValidateCROMAction implements IObjectActionDelegate {

	private Shell shell;
	private ISelection selection;
	
	public ValidateCROMAction() {
		super();
	}
	...
	public final void run(IAction action) {		
		...
		for (IPath p : list) {			
			try {
				...
				String pathIORM = p.toString().replace(".crom", ".crom_diagram");
				Resource resourceIORM = resourceSet.getResource(URI.createURI(pathIORM), true);
				...
				Job job = new WorkspaceJob("Validating CROM...") {
					public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {					
						...
						validator.validate(p, resourceCROM, resourceIORM);
